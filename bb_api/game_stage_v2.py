...
def games_in_range(firstDate, lastDate):
    # because it's going in public repository
    ...
    try:
        (first,last)=(firstDate.replace("-",""), lastDate.replace("-",""))
        response = requests.get(
            url=pullurl,
            params={
                "date": f"from-{first}-to-{last}"
            },
            headers={
...
response = games_in_range("2018-06-24", "2018-06-24")
if response != None:
